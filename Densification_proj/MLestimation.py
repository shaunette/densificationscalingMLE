...
def raw_estimators(estimates):
    ...
    """
    parcombs = configmle.parcombs2
    kappa_seq = []
    np_seq = []
    for combs in estimates["params"]:
        kappa,np = parcombs[combs]#parcombs2[combs]
...
def estimates(NM):
    ...
    """
    likelihood_funcs = configmle.Likelihoodfns    
    knp_ests = []
    for elem in NM:
        N,M = elem
        prob_NM = []
        combid = []
        for fid,LF in likelihood_funcs.items():#Likelihoodfns.items():
    ...
    kappavalues, npvalues = raw_estimators(estimates)#, parcombs2)
